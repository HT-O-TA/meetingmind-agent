...
阶段 4 已接入一种真实 ASR：本地 `FunASR 1.4.3`，默认配置为 `paraformer-zh + fsmn-vad + ct-punc + CAM++`。正式入口只接受 WAV，并通过 RabbitMQ 异步处理。转写结果会持久化到会议正文、发言记录和版本化元数据中；旧 OpenAI Whisper、多模态视频和 Qwen-VL 路径仍为默认关闭的历史骨架，不计入完成能力。
...
flowchart LR
    ...
    W --> F[FunASR 懒加载推理]
    F --> E[ASR Evidence v1]
    E --> M[meetings.raw_transcript + metadata]
    E --> S[speech_records 时间戳/匿名说话人]
    E --> D[规则纪要初稿 + 待办候选]
    M --> T[Redis 任务结果]
    S --> T
    D --> T

...
> **阶段证据说明**：本文保留阶段 5 的训练运行、数据卡和对比结果。阶段 7 已删除当时未被正式评估引用的旧转写语料副本，合成微调数据与可复现脚本仍保留。
...
## 2. 为什么当时没有直接拿 217 份转写训练

阶段 5 时，工作区的转写被项目文档标注为 AliMeeting 公开语料，但没有原始 meeting key 与 Action Item Detection 真值映射。官方 AliMeeting4MUG 确实提供 AID 赛道，官方代码的下载函数要求 ModelScope 个人 Token；当时的仓库示例只有 5 个 meeting key 的伪提交文件，不构成训练集。这些未被正式评估使用的副本已在阶段 7 删除。
